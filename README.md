...
The Optimism L2 is a slightly modified Geth L1, and is based on Rojo Loco (v1.9.10) (Jan 20, 2020).

## Critical issues

Since this is all based on Geth v.1.9.10, the tests will not work unless you check out a stale branch of the tests after `git submodule update`: 

```

git submodule update --init --recursive
git checkout v7.0.0-beta.1 //the test suite relevant to Geth v.1.9.10
...
To test the code using sensible defaults in Sequencer mode, run:
...
If you do not have have the data service up at :7878, then the L2 will fail after 
trying to boot with `Error starting protocol stack: Cannot initialize syncservice` 
...
docker build . --file Dockerfile.test --tag geth_test

```

Run Lint & Test Docker Image:
```bash

docker run geth_test:latest

```

### Build and Run the Sequencer/Verifier via LOCAL Docker

To build a local Sequencer docker image:

```bash

docker build . --file Dockerfile.l2_local_sequencer --tag geth_sequencer
docker run geth_sequencer # will fail if you have not set the .env variables

```

### Build and Run the Verifier via LOCAL Docker

To build a local Verifier docker image:

```bash

docker build . --file Dockerfile.l2_local_verifier --tag geth_verifier
docker run geth_verifier # will fail if you have not set the .env variables

```

The only difference compared to the sequencer is that the `--rollup.verifier` flag has been set.

### Build a DockerHub L2 (which can either be a sequencer or a verifier, depending on flags you pass)

To build the L2 docker image:

```bash

docker build . --file Dockerfile.l2 --tag omgx/go-ethereum:latest
docker run omgx/go-ethereum:latest
docker push omgx/go-ethereum:latest
...
Running a sequencer requires the [Data Transport Layer](https://github.com/ethereum-optimism/data-transport-layer) to be synced. 
...
This may be suitable for simple use cases. Users that need more flexibility
...
or, also locally, in the `/regenesis` folders.

To use a different genesis state, pass in a path to one of the JSON files in the repository. 
The genesis configuration system accepts either local files or http addresses, and will figure out 
what to do based on the string provided. 
